...
export class RedDeVeterinarias {
    ...
    eliminarProveedorConsola(nombre: string, telefono: number) {
        const proveedor = this.proveedores.find(p => p.getNombre() === nombre && p.getTelefono() === telefono);
        if (proveedor) {
            const index = this.proveedores.indexOf(proveedor);
            this.proveedores.splice(index, 1);
    ...
    eliminarVeterinariaConsola(nombre: string, direccion: string) {
    ...
    listarPacientes() {
        this.pacientes.forEach(paciente => {
            console.log(`${paciente.getIdDueno()}, ${paciente.getNombre()}, ${paciente.getEspecie()}`);
        });
    }
